...
const getSingleUser = async (req, res) => {
    ...
    try {
        if (!mongoose.Types.ObjectId.isValid(id)) return res.status(404).json({error: true, message: "The user ID is invalid!" })
...
const deleteUser = async (req, res) => {
	...
	try {
        ...
        // check if the user has an existing products in the cart, favorites or shops
        const existingCart = await Cart.findOne({buyerId: user._id})
        const existingFavourites = await Favourites.findOne({buyerId: user._id})
        const sellerShop = await Shop.findOne({sellerId: user._id})
            
        if (!existingCart || !existingFavourites || !sellerShop) {
            ...
            // delete the shop
            const deletedShop = await Shop.findOneAndDelete({sellerId: user._id})

            const deletedUser = await User.findByIdAndDelete({_id: id})
            return res.status(200).json({
                success: 'true', 
                message: 'User has been deleted successfully', 
                deletedShopProducts: shopProducts, 
...
const updatePassword = async (req, res) => {
    ...
    try {
        ...
        if (!currentPassword || !newPassword || !confirmPassword) return res.status(404).json({error: true, message: "All fields are required!" })
        ...
        if (!isPasswordMatch) return res.status(401).json({error: true, message: 'Incorrect current password' });
